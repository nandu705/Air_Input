...
['A','S','D','F','G','H','J','K','L'],
['Z','X','C','V','B','N','M'],
['SPACE','BACK','EXIT']  # BACK key added
]
...
def draw_keyboard(img):
    ...
    for i,row in enumerate(keys):
        ...
        for key in row:
            ...
            display_text = key
            if key == "BACK":
                display_text = "BACK"  # display as BACK

            if key == "SPACE":
                w = key_w * 4
            elif key == "EXIT" or key == "BACK":
                w = key_w * 2
            else:
                w = key_w
            ...
            cv2.rectangle(img,(x,y),(x+w,y+key_h),(255,0,255),2)
            cv2.putText(img,display_text,(x+6,y+30),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.6,(255,255,255),2)
        ...
        for key,x,y,w,h in key_locations:
            ...
            if x < index_x < x+w and y < index_y < y+h:
                ...
                if distance < 40 and typed_delay == 0:
                    ...
                    if key == "SPACE":
                        pyautogui.write(" ")
                    elif key == "BACK":  # Backspace functionality
